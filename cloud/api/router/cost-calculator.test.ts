...
describe("CostCalculator", () => {
  ...
  describe("OpenAICostCalculator", () => {
    ...
    it.effect("should extract usage from OpenAI Responses API response", () =>
      Effect.gen(function* () {
        ...
        assert(result !== null);
        expect(result.cost.totalCost).toBeGreaterThan(0);
        expect(result.formattedCost.input).toBeDefined();
        expect(result.formattedCost.output).toBeDefined();
        expect(result.formattedCost.total).toBeDefined();
      }),
    );
  ...
  describe("formatted cost output", () => {
    it.effect("should format costs correctly", () =>
      Effect.gen(function* () {
        const calculator = new OpenAICostCalculator();
        const responseBody = {
          usage: {
        ...
        // Calculate and verify formatted cost
        const result = yield* calculator.calculate("gpt-4o-mini", usage);
        assert(result !== null);
