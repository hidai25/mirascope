 ...
 * Uses ClickHouseClient for database access with automatic connection pooling.
 *
 * ## Architecture
 *
 * ```
 * ClickHouseSearchService (read-only search operations)
 *   └── data access via ClickHouseClient
 * ```
 *
 * ## Query Strategies
 *
...
/**
 * Converts SqlError to ClickHouseError for consistent error handling.
 */
...
export interface ClickHouseSearchServiceInterface {
...
export class ClickHouseSearchService extends Context.Tag(
  "ClickHouseSearchService",
)<ClickHouseSearchService, ClickHouseSearchServiceInterface>() {
      ...
      return {
        search: (input: SpanSearchInput) =>
          Effect.gen(function* () {
            ...
            // Build WHERE clause as Fragment
            const whereClause = buildSearchWhereClause(sql, validatedInput);
            ...
            const orderByFragment = sql.literal(orderBy);
            ...
            const rows = yield* sql<SpanSummaryRow>`
              SELECT *
              FROM (
                SELECT
                  s.environment_id,
              ...
              -- ClickHouse requires LIMIT/OFFSET as literal integers, not parameters
        ...
        getTraceDetail: (input: TraceDetailInput) =>
          Effect.gen(function* () {
            ...
            const rows = yield* sql<SpanDetailRow>`
              SELECT
                id,
                trace_db_id,
                trace_id,
              ...
              WHERE environment_id = toUUID(${input.environmentId})
        ...
        getAnalyticsSummary: (input: AnalyticsSummaryInput) =>
          Effect.gen(function* () {
            ...
            const baseConditions: Array<string | Statement.Fragment> = [
              sql`s.environment_id = toUUID(${input.environmentId})`,
              sql`s.start_time >= toDateTime64(${formatDateTime64(input.startTime)}, 9)`,
              sql`s.start_time <= toDateTime64(${formatDateTime64(input.endTime)}, 9)`,
            ];

            if (input.functionId) {
              baseConditions.push(
                sql`s.function_id = toUUID(${input.functionId})`,
              );
            }

            const baseWhere = sql.and(baseConditions);
            ...
            const analyticsRows = yield* sql<AnalyticsRow>`
              SELECT
            ...
            const modelConditions = [
              ...baseConditions,
              sql`s.model IS NOT NULL`,
            ];
            const modelWhere = sql.and(modelConditions);

            const topModelRows = yield* sql<TopModelRow>`
            ...
            const functionConditions = [
              ...baseConditions,
              sql`s.function_name IS NOT NULL`,
            ];
            const functionWhere = sql.and(functionConditions);

            const topFunctionRows = yield* sql<TopFunctionRow>`
...
/**
 * Build WHERE clause for search query using Effect SQL fragments.
 * Returns a Statement.Fragment that can be embedded in sql`` templates.
 */
function buildSearchWhereClause(
  sql: SqlClient.SqlClient,
  input: SpanSearchInput,
): Statement.Fragment {
  const conditions: Array<string | Statement.Fragment> = [];

  // Required filters (all column references use 's.' prefix)
  // Use toUUID() for explicit type conversion (ClickHouse UUID type)
  conditions.push(sql`s.environment_id = toUUID(${input.environmentId})`);
  conditions.push(
    sql`s.start_time >= toDateTime64(${formatDateTime64(input.startTime)}, 9)`,
  );
  conditions.push(
  ...
  if (input.query) {
    const tokens = input.query.trim().toLowerCase().split(/\s+/);
    for (const token of tokens) {
      conditions.push(sql`hasToken(s.name_lower, ${token})`);
    }
  }

  // Optional filters
  if (input.traceId) {
    conditions.push(sql`s.trace_id = ${input.traceId}`);
  }

  if (input.spanId) {
    conditions.push(sql`s.span_id = ${input.spanId}`);
  }

  if (input.model && input.model.length > 0) {
    conditions.push(sql.in("s.model", input.model));
  }

  if (input.provider && input.provider.length > 0) {
    conditions.push(sql.in("s.provider", input.provider));
  }

  if (input.functionId) {
    conditions.push(sql`s.function_id = toUUID(${input.functionId})`);
  }

  if (input.functionName) {
    conditions.push(sql`s.function_name = ${input.functionName}`);
  }

  if (input.hasError === true) {
    conditions.push(sql`s.error_type IS NOT NULL`);
  } else if (input.hasError === false) {
    conditions.push(sql`s.error_type IS NULL`);
  }

  if (input.minTokens !== undefined) {
    conditions.push(sql`s.total_tokens >= ${input.minTokens}`);
  }

  if (input.maxTokens !== undefined) {
    conditions.push(sql`s.total_tokens <= ${input.maxTokens}`);
  }

  if (input.minDuration !== undefined) {
    conditions.push(sql`s.duration_ms >= ${input.minDuration}`);
  }

  if (input.maxDuration !== undefined) {
    conditions.push(sql`s.duration_ms <= ${input.maxDuration}`);
  }
  ...
  if (input.attributeFilters) {
    for (const filter of input.attributeFilters) {
      switch (filter.operator) {
        case "eq":
          conditions.push(
            sql`JSONExtractString(s.attributes, ${filter.key}) = ${filter.value ?? ""}`,
          );
          break;
        case "neq":
          conditions.push(
        ...
        case "contains":
          conditions.push(
            sql`JSONExtractString(s.attributes, ${filter.key}) LIKE ${`%${filter.value ?? ""}%`}`,
          );
          break;
        case "exists":
          conditions.push(sql`JSONHas(s.attributes, ${filter.key})`);
          break;
      }
    }
  }
